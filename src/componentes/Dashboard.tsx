...
export default function Dashboard({
  ...
  useEffect(() => {
    try {
      ...
      setLocalTotales(metas.length)
      setLocalCompletados(metas.filter((m: any) => m.status === "completado").length)
      setLocalParciales(metas.filter((m: any) => m.status === "parcial").length)

      const last = metas.filter((m: any) => m.status === "completado").slice(-1)[0]
      if (last) setLocalUltimo(last)
    } catch {}
  }, [])
