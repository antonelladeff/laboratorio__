...
export default function Dashboard({ completados = 0, totales = 0, ultimoCompletado = null }: { completados?: number, totales?: number, ultimoCompletado?: UltimoCompletado }) {
    ...
    const formattedDateRaw = dateFormatter.format(now)
    ...
    return (
        <main className="flex-1 h-screen bg-gray-50 overflow-auto">
            <div className="container mx-auto px-6 py-8">
                {/* Encabezado de bienvenida con fecha y hora */}
                <div className="mb-8">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">¡Bienvenido!</h1>
                            <p className="text-gray-600 mt-1">Panel de control del laboratorio</p>
                        </div>
                        <div className="flex flex-wrap gap-3">
                            <div className="bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-3 min-w-[220px]">
                                <p className="text-xs font-semibold uppercase text-gray-500">Fecha</p>
                                <p className="text-sm font-semibold text-gray-900">{formattedDate}</p>
                            </div>
                            <div className="bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-3 min-w-[180px]">
                                <p className="text-xs font-semibold uppercase text-gray-500">Hora</p>
                                <Clock boxBg="bg-transparent" boxBorder="border-transparent" iconColor="text-blue-500" showDate={false} />
                            </div>
                        </div>
                    </div>
                </div>
